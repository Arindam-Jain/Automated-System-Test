...
const validateSelectItems = require('../checks/csn/selectItems');
...
function getMixinAssocOfQueryIfPublished(query, association, associationName) {
  if (query && query.SELECT && query.SELECT.mixin) {
    ...
    for (const elem of Object.keys(query.SELECT.mixin)) {
      ...
      if (elem === originalName && mixin_element.target === association.target)
 ...
 * - (030) ("plain" / "quoted" only) For all elements, derived types are replaced by their final base type.
...
function transformForHanaWithCsn(inputModel, options) {
  ...
  const cleanup = validator(csn, {
    ...
    /* query validators    */ [ validateMixinOnCondition, validateSelectItems ]);
 ...
 // check unique constraints - further processing is done in rewriteUniqueConstraints`
  ...
  // (030) - only for 'plain' / 'quoted' names
  ...
  if (options.forHana.alwaysResolveDerivedTypes || options.forHana.names === 'plain' || options.forHana.names === 'quoted') {
    forEachDefinition(csn, (artifact) => {
      forEachMemberRecursively(artifact, (member) => {
        toFinalBaseType(member);
      });
      if(artifact.kind === 'type')
        setProp(artifact, '_ignore', true);
    });
  }
  else {
  ...
  function handleAssocToJoins() {
    ...
    forEachDefinition(csn, (art, artName) => {
      ...
      if (art.query && newArt && newArt.query && !isNoDbArtifact(newArt)) {
