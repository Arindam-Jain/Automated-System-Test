...
function _log(level, arg) /* NOSONAR */ {
  const { params } = arg

  // cds.log() called with multiple args or first arg not an object? -> pass through
  if (params.length !== 1 || typeof params[0] !== 'object') {
    LOG[level](arg)
    return
  }

  // single arg of type object
  const obj = params[0]
  // Error.message and Error.stack are not enumerable
  const toLog = Object.assign(
    { id: arg.id, level: arg.level, timestamp: arg.timestamp, message: obj.message, stack: obj.stack },
    obj
  )
  ...
  if (level === 'error') {
    ...
    const numericCode = toLog.statusCode || Number(toLog.code)
