...
const processorFn = (errors, req) => {
  return (rows, key) => {
    ...
    for (const row of rows) row['*@odata.etag'] = row[key]
  }
}
...
const _postProcess = (odataReq, req, result, service) => {
  if (odataReq.getConcurrentResource() !== null) {
    ...
    if (template.elements.size !== 0) {
      templateProcessor(processorFn([], req), result.value, template)
    }
  }
  const functionReturnType = getActionOrFunctionReturnType(
    odataReq.getUriInfo().getPathSegments(),
  ...
  autoExpandToOne(functionReturnType || req.target, result.value || result)
  if (req.target) {
