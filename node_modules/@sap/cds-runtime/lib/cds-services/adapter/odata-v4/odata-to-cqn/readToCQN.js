...
const _setAggregatedAwayPropertiesToNullInFilter = (having, groupBy) => {
  ...
  for (let i = 0; i < having.length; i++) {
    if (having[i].ref && !groupBy.includes(having[i].ref[having[i].ref.length - 1])) {
      having[i] = { val: null }
    }
  }
}
...
const _topSkip = (queryOptions, maxPageSize, cqn) => {
  if (queryOptions && (queryOptions.$top || queryOptions.$skip)) {
    ...
    cqn.limit(Math.min(top, maxPageSize), parseInt(queryOptions.$skip) || 0)
  }
}
...
const readToCQN = (service, target, odataReq) => {
  ...
  // REVISIT: if !isCollectionOrToMany we should also use SELECT.one, but currently breaks tests
  if (!isCollectionOrToMany) {
    cqn.SELECT._one = true
  }
