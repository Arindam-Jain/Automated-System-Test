...
function _entityOrTypeName(navSourceSegment) {
  ...
  if (navSourceSegment.getKind() === 'COMPLEX.PROPERTY') {
    return navSourceSegment.getProperty().getType().getFullQualifiedName().toString()
  }
...
const _getNavigationInfoX4 = (navProperty, navSourceSegment, csn) => {
  // this algorithm is somewhat opportunistic in case property names include _ but should ne fine
  const entityOrTypeName = _entityOrTypeName(navSourceSegment)
  let def = csn.definitions[entityOrTypeName]
  if (!def) {
    const splitted = entityOrTypeName.split('_')
    if (splitted.length > 1) {
      def = csn.definitions[splitted.shift()]
      while (def && splitted.length > 0) {
        let ele = splitted.shift()
        while (!def.elements[ele]) {
          ele = ele + '_' + splitted.shift()
        }
        def = def.elements[ele]
      }
    }
  }
...
const _getNavigationInfo = (navProperty, navSourceSegment, csn) => {
  ...
  const entityOrTypeName = _entityOrTypeName(navSourceSegment)
  const navigationName = navProperty.getName()
  const navigationDefinition = csn.definitions[entityOrTypeName].elements[navigationName]
  return { navigationName, navigationDefinition }
}
