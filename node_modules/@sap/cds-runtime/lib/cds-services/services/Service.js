...
class ApplicationService extends cds.Service {
  ...
  async _calculateDiff(req, providedData) {
    if (req.event === 'CREATE') {
      return this._diffCreate(req, providedData)
    }

    // REVISIT: rm target[PERSISTENCE_SKIP] !== 'if-unused' with cds^4.5
    if (req.target[PERSISTENCE_SKIP] && req.target[PERSISTENCE_SKIP] !== 'if-unused') {
      return
    }

    if (req.event === 'DELETE') {
      return this._diffDelete(req)
    }

    if (req.event === 'UPDATE') {
      return this._diffUpdate(req, providedData)
    }

    if (req.event === 'PATCH') {
      return this._diffPatch(req, providedData)
    }
  }
