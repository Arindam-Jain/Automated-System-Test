...
const _getSelectCQN = (req, keys) => {
  return SELECT.from(ensureNoDraftsSuffix(req.target.query._target.name), [1]).where(keys.keyList)
}

const _getDraftSelectCQN = (req, keys) => {
  const draftEntityName = ensureDraftsSuffix(req.target.name)

  return SELECT.from(draftEntityName, ['DraftUUID', { ref: ['DRAFT_DraftAdministrativeData', 'InProcessByUser'] }])
...
const _validate = (activeResult, draftResult, req, IsActiveEntity) => {
  ...
  if (draftResult.length !== 0 && draftResult[0].InProcessByUser !== req.user.id) {
