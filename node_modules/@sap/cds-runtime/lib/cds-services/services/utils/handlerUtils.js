...
const getScenario = (entity, serviceEntities) => {
  ...
  const navApprovals = _getNavigationApprovals(entity, serviceEntities)
...
const _getExpandCqnForInstance = (elementKey, data, req, definitions) => {
  ...
  if (target && compData) {
    if (!Array.isArray(compData)) compData = [compData]
    ...
    cqn = cqns[0]
    if (cqns.length > 0) _mergeExpandCQNs(cqn, cqns)
  }
...
const _getExpandCqnForEntity = (elementKey, req, definitions) => {
  ...
  if (cqns.length > 0) _mergeExpandCQNs(cqns[0], cqns)
...
const _columnsNoSkippedNoAssocNoDraft = elements => {
  return Object.keys(elements).filter(
    k =>
      ...
      !elements[k]._isAssociationEffective &&
      // REVISIT: rm target['@cds.persistence.skip'] !== 'if-unused' with cds^4.5
      !(
        elements[k]._target &&
        elements[k]._target['@cds.persistence.skip'] &&
        elements[k]._target['@cds.persistence.skip'] !== 'if-unused'
      )
  )
}

const _columnsAssocToOne = elements =>
  Object.keys(elements).filter(k => elements[k]._isAssociationEffective && elements[k].is2one)
const _addKeysRef = elements => {
  const keys = []
  for (const k of elements) {
    keys.push({ ref: [k.childFieldName] })
  }
  return keys
}
...
const _mergeAssocToOneColumns = (colsAssocToOne, req, cqn) => {
  if (colsAssocToOne.length !== 0) {
    for (const assocCol of colsAssocToOne) {
      ...
      if (assocKeys) {
        const keys = foreignKeyPropagations(req.target.elements[assocCol], true, false)
        if (keys) {
          const assocC = {
            ref: [assocCol],
            expand: _addKeysRef(keys)
          }
...
const getDeepSelect = (req, definitions) => {
  ...
  const comps = cols.filter(k => req.target.elements[k]._isCompositionEffective)
  if (comps.length > 0) {
