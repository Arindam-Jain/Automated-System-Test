...
const _checkAssociations = (element, row, req, result, ref) => {
  const data = _getDataFromRef(row, ref)
  if ((req.event === 'CREATE' || req.event === 'UPDATE') && data !== undefined) {
...
const _checkCreateUpdate = (result, ref, rootEntity, checks, data, req, run) => {
  ...
  return data.reduce((result, row) => {
    const assocError = _checkAssociations(resolvedElement, row, req, result, ref)
...
const _isCompBacklink = (entity, assoc) => {
  for (const element in entity.elements) {
    ...
    if (
      (targetElement.isComposition || targetElement['@odata.contained']) &&
      targetElement.on &&
...
const _checkDelete = (result, key, entity, checks, req, csn, run, data) => {
  ...
  return Object.keys(elements).reduce((result, assoc) => {
    if (
      ...
      elements[assoc][ASSERT_INTEGRITY_ANNOTATION] !== false
    ) {
...
const checkReferenceIntegrity = (entity, data, req, csn, run) => {
  const service = csn.definitions[entity.name.split(/\.\w*$/)[0]]
