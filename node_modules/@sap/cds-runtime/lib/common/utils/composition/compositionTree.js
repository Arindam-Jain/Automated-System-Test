...
const _skipPersistence = (element, definitions) => {
  ...
  return definitions[element.target][PERSISTENCE_SKIP] && definitions[element.target][PERSISTENCE_SKIP] !== 'if-unused'
}
...
const _generateKeysIfNeeded = (entity, data) => {
  for (const keyDefinition of Object.values(entity.keys)) {
    if (keyDefinition.key && keyDefinition.type === 'cds.UUID' && data[keyDefinition.name] === undefined) {
      data[keyDefinition.name] = cds.utils.uuid()
    }
  }
}
...
const selectDeepUpdateData = (definitions, cqn, execute, req, includeAllRootColumns = false) => {
  ...
  return _selectDeepUpdateData({
    ...
    execute,
    includeAllRootColumns,
    singleton: req && req.target && isSingleton(req.target),
    alias
  })
}
...
const _addSubDeepUpdateCQN = ({ definitions, compositionTree, data, selectData, cqns, draft }) => {
  ...
  // REVISIT: rm target[PERSISTENCE_SKIP] !== 'if-unused' with cds^4.5
  if (entity[PERSISTENCE_SKIP] && entity[PERSISTENCE_SKIP] !== 'if-unused') {
    return Promise.resolve()
  }
