...
function _getFlattenedKeys(element) {
  const keys = []
  for (const k of Object.keys(element.elements)) {
    if (element.elements[k].elements) {
      keys.push(..._getFlattenedKeys(element.elements[k]))
    } else {
      keys.push(element.name + '_' + k)
    }
  }
  return keys
}
...
class JoinCQNFromExpanded {
  ...
  _expandedToFlat({ entity, givenColumns, readToOneCQN, tableAlias, toManyTree, defaultLanguage }) {
    ...
    for (const column of givenColumns) {
      ...
      if (this._isExpandToMany(column, entity, navigation)) {
        ...
        this.adjustOrderBy(readToOneCQN, mappings, column, tableAlias)
      }
    }
  ...
  adjustOrderBy(readToOneCQN, mappings, column, tableAlias) {
    const colName = column.as || (column.ref && column.ref[column.ref.length - 1])
    if (readToOneCQN.orderBy && mappings[colName]) {
      readToOneCQN.orderBy.forEach(order => {
  ...
  _getKeyNames(entity) {
    ...
    for (const key of Object.keys(keys)) {
      ...
      } else if (keys[key].elements) {
        // > complex key
        keyNames.push(..._getFlattenedKeys(keys[key]))
