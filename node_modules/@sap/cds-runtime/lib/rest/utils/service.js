...
const _getObjectToLog = (e, correlationId) => {
  const obj = {
    message: e.message,
    request: {
  ...
  if (obj.request.headers.authorization) {
    obj.request.headers.authorization = obj.request.headers.authorization.split(' ')[0] + ' ...'
  }

  return obj
}

const run = async (reqOptions, { destination, jwt, kind }) => {
  ...
  try {
    ...
    LOG._warn && LOG.warn(_getObjectToLog(e, reqOptions.headers && reqOptions.headers['x-correlation-id']))
